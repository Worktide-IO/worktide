...
- ~~**Collision Detection** via Mercure-Presence — Hinweis wenn 2 User dieselbe Conversation öffnen~~ — **erledigt**: voter-gated `POST /v1/conversations/{id}/presence` relayt einen Presence-Frame (die SPA darf nicht direkt zum Hub publishen) auf das Topic `worktide:conversation:{id}:presence`. SPA-Hook `useConversationPresence` (Heartbeat alle 20 s über den Endpoint, best-effort `left` beim Unmount, lokales Ablaufen stiller Peers nach ~50 s, Self-Filter) + Amber-Banner „auch hier: …" in der Inbox-Detailseite (DE/EN). Bewusst **stateless** (kein Server-Presence-Store — Clients gossippen + expiren lokal). Live gegen den dev-Hub verifiziert.
...
- ~~**Phone-Conversation** (manuelles Ticket für Telefonate)~~ — **erledigt**: `POST /v1/conversations/phone` (membership-checked via X-Workspace-Id) legt eine Conversation auf einem per-Workspace, sync-losen `phone`-Channel (capabilities: [] → alle Pull/Push-Crons überspringen ihn) an. Ein **eingehender** Anruf wird zum `InboundEvent` (Timeline-Typ `customer`), ein **ausgehender** zur `OutboundMessage` (`message`) — beide fügen sich in die bestehende Timeline; optionale Contact-/Customer-Verknüpfung (Cross-Workspace-Refs abgewiesen, Phase T). SPA: „Telefonat erfassen"-Dialog in der Inbox (Richtung, Gesprächspartner, Betreff, Notiz, Dauer; DE/EN) → navigiert zur neuen Konversation. Zusätzlich gehärtet: Conversation-`DELETE` ist jetzt **soft** (`SoftDeleteProcessor` + Read-Filter) statt Hard-Delete, damit ein gelöschter Thread seine Inbound/Outbound-Historie nicht verwaist.
...
- ~~**Inbound-Webhook für Mail-Provider mit Webhook-API** (SendGrid, Mailgun, Resend)~~ — **erledigt, generisch statt pro-Provider**: der `GenericInboundMailAdapter` (`adapterCode email_webhook`) nimmt eingehende Mail von **jeder** Quelle über die bestehende `POST /v1/inbound/webhooks/{token}`-Route (PUBLIC_ACCESS). Er liest die zwei Shapes, die praktisch alles abdecken — **rohes RFC822-MIME** (Body oder JSON-`message`/`raw`/`rfc822`/`email`-Feld, base64 bei `base64:true` — exakt wie **Postal**s HTTP-Endpoint postet) **und** **generische JSON-/Form-Felder** (`from/to/subject/text/html/messageId/inReplyTo/references`). Auth: der URL-Token ist die Credential; optional zusätzlich HMAC-SHA256 über `authConfig.signingSecret` (Header konfigurierbar, default `X-Signature`). Threading via bestehenden `MailThreader` (gleiche `sourceMetadata.headers`-Shape), Dedup über Message-ID/Body-Hash, Attachments → FileStorage. SPA: „E-Mail (Inbound-Webhook)"-Kanaltyp im Channel-Dialog mit generiertem Token, kopierbarer Ingest-URL + optionalem Signatur-Secret (DE/EN). Live verifiziert (rohes MIME + Postal-base64-Envelope, Threading Root+Reply → eine Konversation, unbekannter Token → 404, HMAC-Reject). Provider-spezifische Adapter (SendGrid/Mailgun-Signaturschemata) sind bei Bedarf Drop-ins auf demselben Fundament.
...
- ~~**Löschen ist reversibel by default (Soft-Delete)**~~ — **erledigt (global)**: `SoftDeleteRemoveProcessorDecorator` dekoriert den API-Platform `remove_processor` und soft-löscht **jede** `SoftDeletableTrait`-Entität (aktuelle *und künftige* `Delete`-Ops, ohne per-Resource-Wiring); `SoftDeleteExtension` versteckt soft-gelöschte Zeilen aus **allen** Reads trait-basiert (keine Whitelist mehr). Opt-out für Link-/Pivot-Zeilen via `HardDeleteOnly`-Marker (`CustomerProduct`, `ServiceAssignment` — sonst blockierte eine soft-gelöschte Zeile das erneute Verknüpfen desselben Paares bis zum Purge). Retention-Purge `worktide:soft-delete:purge --days=30` (hard-löscht alt-soft-gelöschte Zeilen → begrenzt Wachstum + gibt Unique-Keys frei). Tests: Decorator (soft/hard-opt-out/non-soft) + Extension (trait-Erkennung); live verifiziert (DELETE→204, GET→404, Zeile lebt mit `deletedAt`, aus Collection raus). **Bewusst offen:** Kaskaden-Verstecken (lösche ich ein Projekt, bleiben seine Tasks in ihren Listen sichtbar) — nur per-Beziehung sauber lösbar, kein Datenverlust; Unique-Kollision beim Re-Create desselben Keys innerhalb des Retention-Fensters → sauberer 409 bis zum Purge.
...
2. **Phase C — Helpdesk**: weitgehend komplett — Auto-Reply pro Mailbox ✓, Collision-Detection ✓, Inbound-Webhook (generisch, deckt Postal & Co.) ✓, Phone-Conversation ✓. **Google-Workspace-OAuth ist ans Ende geschoben** (aktuell kein Bedarf — nur bei konkretem Kunden mit Gmail-Postfach; IMAP deckt Gmail über App-Passwörter ohnehin ab). Damit ist der Support-Loop, den Portal-Tickets anstoßen, geschlossen.
