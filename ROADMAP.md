...
- **Löschen ist reversibel by default (Soft-Delete)**: Entities mit `SoftDeletableTrait` müssen beim API-`DELETE` soft-gelöscht werden (`SoftDeleteProcessor` setzt `deletedAt`) **und** aus allen Reads verschwinden (`SoftDeleteExtension`), damit ein versehentliches (Batch-)Delete nicht Daten samt verknüpfter Historie unwiederbringlich vernichtet. **Offen (Folgeschritt):** aktuell nur für **File, Folder, Conversation** verdrahtet — die übrigen ~46 `SoftDeletable`-Entities löschen weiterhin **hart** und `deletedAt` ist dort toter Code. Global ausrollen: `SoftDeleteProcessor` an jede `Delete`-Operation + Aufnahme in die `SoftDeleteExtension`-Whitelist, mit Sorgfalt bei Kaskaden (verknüpfte Kinder) und Unique-Constraints (soft-gelöschte Zeile blockiert sonst eine neue mit gleichem Key). In Tests gießen: „`DELETE` einer `SoftDeletable`-Resource setzt `deletedAt` und die Zeile überlebt".
